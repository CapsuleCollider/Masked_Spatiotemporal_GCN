...
def main(args):
    root_path = '..\\data\\CodeGoat24\\cc400'
    ...
    np.save(os.path.join(root_path, 'ABIDE_pcp/abide.npy'), {'timeseires': np.array(times), "label": np.array(labels),"corr": np.array(corrs),"pcorr": np.array(pcorrs), 'site': np.array(site_list)})
