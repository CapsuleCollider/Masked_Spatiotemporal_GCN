...
from model.S2Attention import S2Attention
from model.SimplifiedSelfAttention import SimplifiedScaledDotProductAttention
from model.EMSA import EMSA
from model.CoordAttention import CoordAtt
...
class GCNPredictor(nn.Module):
    ...
    def forward(self, m, node_feature):
        ...
        for i in range(bz):
            x_in[i] = torch.cat((self.cls, x[i]), 0)  # Concatenate along dimension 0 (vertically)
...
class GraphTransformer(nn.Module):
    ...
    def __init__(self, model_config, roi_num=200, node_feature_dim=360, time_series=512, pool_ratio=0.7):
        ...
        self.ssa = SimplifiedScaledDotProductAttention(d_model=100, h=4)
        self.EMSA = EMSA(d_model=100, d_k=100, d_v=100, h=4, H=2, W=196, ratio=2, apply_transform=True)
        self.Coa = CoordAtt(100, 100)
        # self.S2attention = S2Attention(time_series)

        # Gate mechanism
        self.GRU = nn.GRU(input_size=100, hidden_size=100, num_layers=3)

        self.emb2graph = Embed2GraphByProduct(model_config['embedding_size'], roi_num=roi_num)



        self.predictor = GCNPredictor(node_feature_dim, roi_num=roi_num, pool_ratio=pool_ratio,
                                      gcn_layer=model_config['gcn_layer'])

        self.cbam = CBAM(100)      # Plug-and-play module

        self.bn = torch.nn.BatchNorm1d(roi_num)
        self.pe = PositionalEncoding(d_model=100, roi_num=200)
        self.sigmoid = torch.nn.Sigmoid()
    ...
    def forward(self, t, nodes, pseudo):
                                     ...
                                     #torch.Size([16, 392, 100])

        # m = to_4d(t, 2, 196)    # torch.Size([16, 100, 2, 196])
        # m = self.Coa(m)         # torch.Size([16, 100, 2, 196])
        # m = to_3d(m)            # torch.Size([16, 392, 100])

        m = self.EMSA(t, t, t)  # torch.Size([16, 392, 100])



        m, _ = self.extract(m)      # torch.Size([16, 392, 100])

        # m = to_4d(m, 2, 100)   # torch.Size([16, 100, 2, 100])
        # m = self.cbam(m)   # torch.Size([16, 100, 2, 100])
        # m = to_3d(m)  # torch.Size([16, 200, 100])

        m = self.fc(m)          #torch.Size([16, 200, 8])

        m = self.emb2graph(m)       #torch.Size([16, 200, 200])
